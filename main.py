...
def gui():
    ...
    layout = [ [sg.Text('Domain Digger', font=('Courier 12',22), text_color='black',justification='center')],
            [sg.Text('Select an Input Option', font='Courier 12', text_color='blue')],
                [sg.Combo(['IP List', 'Domain List', 'IP Range'],default_value='IP List',readonly=True,key='-in-')],
                [sg.Button(button_text='Go',key='-go-')]]
    # Create the Window
    window = sg.Window('Domain Digger', layout,size=(500,500)).Finalize()
